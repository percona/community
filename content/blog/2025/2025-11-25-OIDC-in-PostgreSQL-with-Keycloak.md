...
date: "2025-11-18T09:00:00+00:00"
...
This is a nice way to try out the UI and start discovering Keycloak, but it is unfortunately not enough for us:
...
Before we can run Keycloak with HTTPS, we need certificates.

To run this command, first we need to somehow get these certificates.
And to do so, we have two easy options:
...
The following is a sample command that can run non-interactively, without asking additional questions:
...
It generates a certificate that is valid for 10 years, for `localhost`.
The `localhost` part is important:
...
If you plan to run Keycloak in a Docker container but run PostgreSQL directly on the host machine, the above certificate can work:
since Docker exposes the 8443 port used by Keycloak on localhost, both the browser used to complete the login process and PostgreSQL can refer to the issuer as `https://localhost:8443/...`
...
Or alternatively, you can generate a certificate that includes both hostnames, this is what the example configuration does:
...
Now that we have certificates, we need to make sure our systems trust them.
...
Similarly, most software using HTTPS for communications defaults to proper certificate verification, but also allows administrators to either disable the certificate check -- not safe in production, but useful for quick demos like this -- or to manually specify a certificate authority used for verification.

PostgreSQL is not most software, and unfortunately at this point, it doesn't provide such options.
...
With the infrastructure setup out of the way, we can now focus on configuring Keycloak itself.
...
![Creating a new realm in Keycloak](blog/2025/11/keycloak_step1_realm.png)
...
![Creating a user in Keycloak](blog/2025/11/keycloak_step2_user.png)
...
![Setting user credentials in Keycloak](blog/2025/11/keycloak_step3_user.png)
...
![Creating a client in Keycloak](blog/2025/11/keycloak_step4_client.png)
...
Adding a secret only adds complexity, as we also have to specify that during the connection call, but it is supported.
...
![Configuring client authentication flow in Keycloak](blog/2025/11/keycloak_step5_client.png)
...
![Configuring consent screen in Keycloak](blog/2025/11/keycloak_step6_client.png)
...
![Creating a client scope in Keycloak](blog/2025/11/keycloak_step7_scope.png)
...
In the real world, that client could be "psql", or "EditorApp", or anything else that uses PostgreSQL.
...
For example if the identity provider has a claim that directly contains user names matching PostgreSQL usernames, we could use that field instead and skip the manual mapping.
