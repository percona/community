...
It also happens with open source projects, where we can see that drive-by contributors storm projects with AI-created PRs.
...
the minimal effort, most comfortable combination, and that results in a local maximum, not a global one.
...
Yes, it's still hit-and-miss, but isn't it the same for humans, when we start new research prototypes?
...
6. And finally, a summary, an overview from the interesting points of all above, so that I can quickly decide what to expect:
   is this already a ready-to-commit patch I should only read to see what a good patch looks like, or am I reviewing something completely broken?

This is of course highly automatable, and that is where my [autoreviewer](https://github.com/dutow/autoreviewer) comes in:
...
Back when I started it, programmatic use of `claude` was still allowed with subscription pricing, but there were already talks about it being restricted to token-based usage for third-party tools.
...
that takes CPU time (build and test execution on my PC), I would also quickly hit my session limits, and even if I did process everything, I do not have the time to personally look through all that.
